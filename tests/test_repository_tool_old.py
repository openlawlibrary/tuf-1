...
class TestRepository(unittest.TestCase):
    ...
    def test_init(self):
        ...
        repository = repo_tool.Repository(
    ...
    def create_repository_directory(self):
        ...
        targets_directory = os.path.join(
            temporary_directory, "repository", repo_tool.TARGETS_DIRECTORY_NAME
        )
        original_targets_directory = os.path.join(
            "repository_data", "repository", "targets"
        )
    ...
    def test_writeall(self):
        ...
        metadata_directory = os.path.join(
            repository_directory, repo_tool.METADATA_STAGED_DIRECTORY_NAME
        )

        repository = repo_tool.create_new_repository(
        ...
        targets_pubkey_path = os.path.join(
            keystore_directory, "targets_key.pub"
        )
        snapshot_pubkey_path = os.path.join(
            keystore_directory, "snapshot_key.pub"
        )
        timestamp_pubkey_path = os.path.join(
            keystore_directory, "timestamp_key.pub"
        )
        role1_pubkey_path = os.path.join(
            keystore_directory, "delegation_key.pub"
        )

        root_pubkey = repo_tool.import_rsa_publickey_from_file(root_pubkey_path)
        targets_pubkey = repo_tool.import_ed25519_publickey_from_file(
            targets_pubkey_path
        )
        snapshot_pubkey = repo_tool.import_ed25519_publickey_from_file(
            snapshot_pubkey_path
        )
        timestamp_pubkey = repo_tool.import_ed25519_publickey_from_file(
            timestamp_pubkey_path
        )
        role1_pubkey = repo_tool.import_ed25519_publickey_from_file(
            role1_pubkey_path
        )
        ...
        timestamp_privkey_path = os.path.join(
            keystore_directory, "timestamp_key"
        )
        role1_privkey_path = os.path.join(keystore_directory, "delegation_key")

        root_privkey = repo_tool.import_rsa_privatekey_from_file(
            root_privkey_path, "password"
        )
        targets_privkey = repo_tool.import_ed25519_privatekey_from_file(
            targets_privkey_path, "password"
        )
        snapshot_privkey = repo_tool.import_ed25519_privatekey_from_file(
            snapshot_privkey_path, "password"
        )
        timestamp_privkey = repo_tool.import_ed25519_privatekey_from_file(
            timestamp_privkey_path, "password"
        )
        role1_privkey = repo_tool.import_ed25519_privatekey_from_file(
            role1_privkey_path, "password"
        )
        ...
        self.assertRaises(
            tuf.exceptions.UnsignedMetadataError, repository.writeall
        )
        ...
        tuf.roledb.update_roleinfo(
            "targets", targets_roleinfo, repository_name=repository_name
        )
        repository.status()
        ...
        tuf.roledb.update_roleinfo(
            "role1", role1_roleinfo, repository_name=repository_name
        )
        repository.status()
        ...
        self.assertRaises(
            tuf.exceptions.UnsignedMetadataError, repository.writeall, True
        )
        ...
        repository.mark_dirty(
            ["role1", "targets", "root", "snapshot", "timestamp"]
        )
        ...
        self.assertEqual(
            ["root", "timestamp"], tuf.roledb.get_dirty_roles(repository_name)
        )
        repository.unmark_dirty(["root"])
        self.assertEqual(
            ["timestamp"], tuf.roledb.get_dirty_roles(repository_name)
        )
        ...
        # Test improperly formatted arguments.
        self.assertRaises(
            securesystemslib.exceptions.FormatError, repository.writeall, 3
        )
    ...
    def test_writeall_no_files(self):
        ...
        targets_directory = os.path.join(
            repository_directory, repo_tool.TARGETS_DIRECTORY_NAME
        )
        ...
        target2_hashes = {
            "sha256": "517c0ce943e7274a2431fa5751e17cfd5225accd23e479bfaad13007751e87ef"
        }
        target1_fileinfo = tuf.formats.make_targets_fileinfo(
            555, target1_hashes
        )
    ...
    def test_get_filepaths_in_directory(self):
        ...
        metadata_directory = os.path.join(
            "repository_data", "repository", "metadata"
        )
        ...
        metadata_files = repo.get_filepaths_in_directory(
            metadata_directory, recursive_walk=True
        )
        self.assertEqual(sorted(expected_files), sorted(metadata_files))
        ...
        self.assertRaises(
            securesystemslib.exceptions.FormatError,
            repo.get_filepaths_in_directory,
            3,
        ...
        temporary_directory = tempfile.mkdtemp(dir=self.temporary_directory)
        nonexistent_directory = os.path.join(
            temporary_directory, "nonexistent/"
        )
        self.assertRaises(
            securesystemslib.exceptions.Error,
            repo.get_filepaths_in_directory,
    ...
    def test_writeall_abstract_storage(self):
        ...
        repository = repo_tool.create_new_repository(
            repository_directory, repository_name, storage_backend
        )
...
class TestMetadata(unittest.TestCase):
    def setUp(self):
        ...
        class MetadataRole(repo_tool.Metadata):
            def __init__(self):
                ...
                tuf.roledb.add_role(
                    self._rolename, roleinfo, repository_name="test_repository"
                )
    ...
    def test_expiration(self):
        ...
        expired_datetime = tuf.formats.unix_timestamp_to_datetime(
            int(time.time() - 1)
        )
        try:
    ...
    def test_keys(self):
        ...
        key_path = os.path.join(
            "repository_data", "keystore", "snapshot_key.pub"
        )
    ...
    def test_signing_keys(self):
        ...
        key_path = os.path.join("repository_data", "keystore", "root_key")
        key_object = repo_tool.import_rsa_privatekey_from_file(
            key_path, "password"
        )
    ...
    def test_add_verification_key(self):
        ...
        self.assertRaises(
            securesystemslib.exceptions.Error,
            self.metadata.add_verification_key,
            key_object,
            expired,
        )
    ...
    def test_remove_verification_key(self):
        ...
        self.assertRaises(
            securesystemslib.exceptions.FormatError,
            self.metadata.remove_verification_key,
            3,
        )

        # Test non-existent public key argument.
        key_path = os.path.join(
            "repository_data", "keystore", "targets_key.pub"
        )
        unused_key_object = repo_tool.import_ed25519_publickey_from_file(
            key_path
        )
    ...
    def test_load_signing_key(self):
        # Test normal case.
        key_path = os.path.join("repository_data", "keystore", "snapshot_key")
        key_object = repo_tool.import_ed25519_privatekey_from_file(
            key_path, "password"
        )
        self.metadata.load_signing_key(key_object)
        ...
        self.assertRaises(
            securesystemslib.exceptions.FormatError,
            self.metadata.load_signing_key,
            3,
        )
    ...
    def test_unload_signing_key(self):
        ...
        self.assertRaises(
            securesystemslib.exceptions.FormatError,
            self.metadata.unload_signing_key,
            3,
        )

        # Test non-existent key argument.
        key_path = os.path.join("repository_data", "keystore", "targets_key")
        unused_key_object = repo_tool.import_ed25519_privatekey_from_file(
            key_path, "password"
        )
    ...
    def test_add_signature(self):
        ...
        tuf.roledb.create_roledb_from_root_metadata(
            root_signable["signed"], repository_name="test_repository"
        )
    ...
    def test_remove_signature(self):
        ...
        self.assertRaises(
            securesystemslib.exceptions.FormatError,
            self.metadata.remove_signature,
            3,
        )
        ...
        targets_signable = securesystemslib.util.load_json_file(
            targets_filepath
        )
...
class TestTargets(unittest.TestCase):
    ...
    def setUp(self):
        ...
        tuf.keydb.create_keydb("test_repository")
        temporary_directory = tempfile.mkdtemp(dir=self.temporary_directory)
        self.targets_directory = os.path.join(
            temporary_directory, "repository", "targets"
        )
        original_targets_directory = os.path.join(
            "repository_data", "repository", "targets"
        )
        shutil.copytree(original_targets_directory, self.targets_directory)
        self.targets_object = repo_tool.Targets(
            self.targets_directory, repository_name="test_repository"
        )
    ...
    def test_init(self):
        ...
        targets_object = repo_tool.Targets(
            "targets_directory/", "package", roleinfo
        )
        self.assertTrue(isinstance(targets_object, repo_tool.Metadata))
        ...
        # Test improperly formatted arguments.
        self.assertRaises(
            securesystemslib.exceptions.FormatError, repo_tool.Targets, 3
        )
        self.assertRaises(
            securesystemslib.exceptions.FormatError,
            repo_tool.Targets,
    ...
    def test_call(self):
        ...
        public_key = repo_tool.import_ed25519_publickey_from_file(
            public_keypath
        )
        ...
        self.assertRaises(
            tuf.exceptions.UnknownRoleError, targets_object, "unknown_role"
        )

        # Test improperly formatted argument.
        self.assertRaises(
            securesystemslib.exceptions.FormatError, targets_object, 1
        )
    ...
    def test_add_delegated_role(self):
        ...
        self.assertRaises(
            securesystemslib.exceptions.FormatError,
            self.targets_object.add_delegated_role,
            "targets",
            "bad_object",
        )
    ...
    def test_add_target(self):
        ...
        target2_fullpath = os.path.join(
            self.targets_directory, target2_filepath
        )
        ...
        self.targets_object.add_target(
            target2_filepath, custom_file_permissions
        )
        ...
        self.assertEqual(
            self.targets_object.target_files["file2.txt"]["custom"],
        ...
        self.targets_object.add_target(
            target2_filepath, custom_file_permissions2
        )
        self.assertEqual(
            self.targets_object.target_files[target2_filepath]["custom"],
    ...
    def test_add_targets(self):
        ...
        self.assertEqual(
            self.targets_object.target_files,
        ...
        self.assertRaises(
            securesystemslib.exceptions.FormatError,
            self.targets_object.add_targets,
            3,
        )
    ...
    def test_delegate(self):
        ...
        self.assertEqual(self.targets_object.get_delegated_rolenames(), ["tuf"])
        ...
        self.targets_object.delegate(
            ...
            threshold,
            terminating=False,
            list_of_targets=["non-existent.txt"],
            path_hash_prefixes=path_hash_prefixes,
        )
    ...
    def test_delegate_hashed_bins(self):
        ...
        def check_prefixes_match_range():
            roleinfo = tuf.roledb.get_roleinfo(
                self.targets_object.rolename, "test_repository"
            )
            have_prefixes = False
        ...
        self.targets_object.delegate_hashed_bins(
            list_of_targets, public_keys, number_of_bins=16
        )
        ...
        self.assertEqual(
            sorted(self.targets_object.get_delegated_rolenames()),
        ...
        self.targets_object.delegate_hashed_bins(
            list_of_targets, public_keys, number_of_bins=512
        )
        check_prefixes_match_range()
    ...
    def test_add_target_to_bin(self):
        ...
        empty_targets_role = repo_tool.Targets(
            self.targets_directory, "empty", repository_name=repository_name
        )
        ...
        rolename = self.targets_object.add_target_to_bin(
            target2_filepath, 16, fileinfo=target2_fileinfo
        )
    ...
    def test_remove_target_from_bin(self):
        ...
        # Add 'target1_filepath' and verify that the relative path of
        # 'target1_filepath' is added to the correct bin.
        added_rolename = self.targets_object.add_target_to_bin(
            target1_filepath, 16
        )
        ...
        self.assertRaises(
            securesystemslib.exceptions.FormatError,
            self.targets_object.remove_target_from_bin,
            3,
            "foo",
        )
    ...
    def test_add_paths(self):
        ...
        targets_object_roleinfo = tuf.roledb.get_roleinfo(
            self.targets_object.rolename, "test_repository"
        )
    ...
    def test_check_path(self):
        ...
        self.assertRaises(
            securesystemslib.exceptions.FormatError,
            self.targets_object._check_path,
            3,
        )
...
class TestRepositoryToolFunctions(unittest.TestCase):
    ...
    def test_create_new_repository(self):
        ...
        repository = repo_tool.create_new_repository(
            repository_directory,
            repository_name,
            use_timestamp_length=True,
            use_timestamp_hashes=True,
            use_snapshot_length=True,
        ...
        # test case is run.
        repository = repo_tool.create_new_repository(
            repository_directory, "my-repo"
        )

        # Test improperly formatted arguments.
        self.assertRaises(
            securesystemslib.exceptions.FormatError,
            repo_tool.create_new_repository,
            3,
            repository_name,
        )
        ...
        tuf.repository_tool.TARGETS_DIRECTORY_NAME = original_targets_directory

    def test_load_repository(self):
        # Test normal case.
        temporary_directory = tempfile.mkdtemp(dir=self.temporary_directory)
        original_repository_directory = os.path.join(
            "repository_data", "repository"
        )

        repository_directory = os.path.join(temporary_directory, "repository")
        metadata_directory = os.path.join(
            repository_directory, "metadata.staged"
        )
        ...
        self.assertTrue(isinstance(repository, repo_tool.Repository))
        self.assertTrue(
            isinstance(repository.targets("role1"), repo_tool.Targets)
        )
        self.assertTrue(
            isinstance(repository.targets("role1")("role2"), repo_tool.Targets)
        )
        ...
        self.assertEqual(
            role1_fileinfo, repository.targets("role1").target_files
        )
    ...
    def test_dirty_roles(self):
        ...
        repository = repo_tool.load_repository(
            original_repository_directory, repository_name
        )
    ...
    def test_dump_signable_metadata(self):
        ...
        metadata_content = repo_tool.dump_signable_metadata(
            targets_metadata_file
        )

        # Test for an invalid targets metadata file..
        self.assertRaises(
            securesystemslib.exceptions.FormatError,
            repo_tool.dump_signable_metadata,
            1,
        )
        self.assertRaises(
            securesystemslib.exceptions.StorageError,
    ...
    def test_append_signature(self):
        ...
        temporary_directory = tempfile.mkdtemp(dir=self.temporary_directory)
        tmp_targets_metadata_path = os.path.join(
            temporary_directory, "targets.json"
        )
        shutil.copyfile(targets_metadata_path, tmp_targets_metadata_path)

        # Test for normal case.
        targets_metadata = securesystemslib.util.load_json_file(
            tmp_targets_metadata_path
        )
