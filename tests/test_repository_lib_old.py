...
class TestRepositoryToolFunctions(unittest.TestCase):
    ...
    def test_import_rsa_privatekey_from_file(self):
        ...
        imported_rsa_key = repo_lib.import_rsa_privatekey_from_file(
        ...
        nonexistent_keypath = os.path.join(
            temporary_directory, "nonexistent_keypath"
        )
        self.assertRaises(
    ...
    def test_import_ed25519_privatekey_from_file(self):
        ...
        imported_ed25519_key = repo_lib.import_ed25519_privatekey_from_file(
            ed25519_keypath, "pw"
        )
        self.assertTrue(
    ...
    def test_get_top_level_metadata_filenames(self):
        ...
        filenames = {
            "root.json": metadata_directory + "root.json",
        ...
        self.assertRaises(
            securesystemslib.exceptions.FormatError,
            repo_lib.get_top_level_metadata_filenames,
            3,
        )
    ...
    def test_get_targets_metadata_fileinfo(self):
        ...
        sha256_digest_object = securesystemslib.hash.digest_filename(
            test_filepath
        )
        sha512_digest_object = securesystemslib.hash.digest_filename(
            test_filepath, algorithm="sha512"
        )
        file_hashes = {
            "sha256": sha256_digest_object.hexdigest(),
    ...
    def test_get_target_hash(self):
        ...
        for filepath, target_hash in expected_target_hashes.items():
            ...
            self.assertTrue(
                securesystemslib.formats.HASH_SCHEMA.matches(target_hash)
            )
            self.assertEqual(repo_lib.get_target_hash(filepath), target_hash)

        # Test for improperly formatted argument.
        self.assertRaises(
            securesystemslib.exceptions.FormatError, repo_lib.get_target_hash, 8
        )
    ...
    def test_generate_root_metadata(self):
        ...
        root_filepath = os.path.join(
            "repository_data", "repository", "metadata", "root.json"
        )
        ...
        root_metadata = repo_lib.generate_root_metadata(
            1, expires, consistent_snapshot=False
        )
        self.assertTrue(tuf.formats.ROOT_SCHEMA.matches(root_metadata))

        root_keyids = tuf.roledb.get_role_keyids("root")
        tuf.keydb._keydb_dict["default"][root_keyids[0]][
            "keytype"
        ] = "bad_keytype"
        self.assertRaises(
            securesystemslib.exceptions.Error,
    ...
    def test_generate_targets_metadata(self):
        ...
        target_files = {
            "file.txt": {"custom": {"file_permission": file_permissions}}
        }
        ...
        targets_metadata = repo_lib.generate_targets_metadata(
            targets_directory,
            target_files,
            version,
            expiration_date,
        ...
        self.assertTrue(
            len(list_targets_directory) + 1, len(new_list_targets_directory)
        )

        # Verify that an exception is not raised if the target files already exist.
        repo_lib.generate_targets_metadata(
            targets_directory,
            target_files,
            version,
            expiration_date,
    ...
    def _setup_generate_snapshot_metadata_test(self):
        ...
        targets_directory = os.path.join(
            repository_directory, repo_lib.TARGETS_DIRECTORY_NAME
        )
        ...
        invalid_metadata_file = os.path.join(
            metadata_directory, "role_file.xml"
        )
        with open(invalid_metadata_file, "w") as file_object:
            file_object.write("bad extension on metadata file")

        return metadata_directory, version, expiration_date, storage_backend
    ...
    def test_generate_snapshot_metadata_with_length(self):
        ...
        for metadata_filename in sorted(
            os.listdir(metadata_directory), reverse=True
        ):

            # In the metadata_directory, there are files with format:
            # 1.root.json. The prefix number should be removed.
            stripped_filename, version = repo_lib._strip_version_number(
                metadata_filename, consistent_snapshot=True
            )
            ...
            if stripped_filename.endswith(".json"):
                if stripped_filename not in TOP_LEVEL_METADATA_FILES:
                    ...
                    self.assertIn(
                        "length", metadata_files_info_dict[stripped_filename]
                    )
                    self.assertNotIn(
                        "hashes", metadata_files_info_dict[stripped_filename]
                    )
    ...
    def _setup_generate_timestamp_metadata_test(self):
        ...
        snapshot_file_path = os.path.join(
            metadata_directory, repo_lib.SNAPSHOT_FILENAME
        )
        ...
        repository_junk = repo_tool.load_repository(
    ...
    def test_generate_timestamp_metadata(self):
        (
            ...
            storage_backend,
            repository_name,
        ) = self._setup_generate_timestamp_metadata_test()
    ...
    def test_sign_metadata(self):
        ...
        root_metadata = securesystemslib.util.load_json_file(root_filename)[
            "signed"
        ]
        ...
        root_private_key = repo_lib.import_rsa_privatekey_from_file(
            root_private_keypath, "password"
        )
        ...
        repo_lib.sign_metadata(
            targets_metadata, targets_keyids, targets_filename, repository_name
        )

        # Add an invalid keytype to one of the root keys.
        root_keyid = root_keyids[0]
        tuf.keydb._keydb_dict[repository_name][root_keyid][
            "keytype"
        ] = "bad_keytype"
        self.assertRaises(
            securesystemslib.exceptions.Error,
    ...
    def test_create_tuf_client_directory(self):
        ...
        self.assertRaises(
            (OSError, tuf.exceptions.RepositoryError),
            repo_lib.create_tuf_client_directory,
            repository_directory,
            "/",
        )
    ...
    def test__generate_and_write_metadata(self):
        ...
        tuf.roledb.create_roledb_from_root_metadata(
            root_signable["signed"], repository_name
        )
        tuf.keydb.create_keydb_from_root_metadata(
            root_signable["signed"], repository_name
        )
        ...
        targets_metadata = os.path.join(
            "repository_data", "repository", "metadata", "targets.json"
        )
        obsolete_metadata = os.path.join(
            metadata_directory, "obsolete_role.json"
        )
        ...
        targets_private_key = repo_lib.import_ed25519_privatekey_from_file(
            targets_private_keypath, "password"
        )
        tuf.keydb.remove_key(
            targets_private_key["keyid"], repository_name=repository_name
        )
        ...
        tuf.roledb.add_role(
            "obsolete_role", targets_roleinfo, repository_name=repository_name
        )
        ...
        snapshot_filepath = os.path.join(
            "repository_data", "repository", "metadata", "snapshot.json"
        )
        snapshot_signable = securesystemslib.util.load_json_file(
            snapshot_filepath
        )
        tuf.roledb.remove_role("obsolete_role", repository_name)
        self.assertTrue(
            os.path.exists(
                os.path.join(metadata_directory, "obsolete_role.json")
            )
        )
        tuf.repository_lib._delete_obsolete_metadata(
            metadata_directory,
        ...
        self.assertFalse(
            os.path.exists(metadata_directory + "obsolete_role.json")
        )
        shutil.copyfile(targets_metadata, obsolete_metadata)
    ...
    def test__delete_obsolete_metadata(self):
        ...
        role1_filepath = os.path.join(
            "repository_data", "repository", "metadata", "role1.json"
        )
        shutil.copyfile(
            role1_filepath, os.path.join(metadata_directory, "role2.json")
        )
        ...
        self.assertTrue(
            os.path.exists(os.path.join(metadata_directory, "root.json"))
        )
    ...
    def test__load_top_level_metadata(self):
        ...
        shutil.copytree(
            os.path.join("repository_data", "repository", "metadata"),
            metadata_directory,
        )
        shutil.copytree(
            os.path.join("repository_data", "repository", "targets"),
            targets_directory,
        )

        # Add a duplicate signature to the Root file for testing purposes).
        root_file = os.path.join(metadata_directory, "root.json")
        signable = securesystemslib.util.load_json_file(
        ...
        storage_backend = securesystemslib.storage.FilesystemBackend()
        repo_lib.write_metadata_file(
            signable, root_file, 8, False, storage_backend
        )

        filenames = repo_lib.get_top_level_metadata_filenames(
            metadata_directory
        )
        repository = repo_tool.create_new_repository(
            repository_directory, repository_name
        )
        repo_lib._load_top_level_metadata(
    ...
    def test__remove_invalid_and_duplicate_signatures(self):
        ...
        root_signable = securesystemslib.util.load_json_file(root_filepath)
        key_filepath = os.path.join("repository_data", "keystore", "root_key")
        root_rsa_key = repo_lib.import_rsa_privatekey_from_file(
            key_filepath, "password"
        )
        ...
        data = securesystemslib.formats.encode_canonical(
            root_signable["signed"]
        ).encode("utf-8")
        new_pss_signature = securesystemslib.keys.create_signature(
            root_rsa_key, data
        )
        root_signable["signatures"].append(new_pss_signature)

        expected_number_of_signatures = len(root_signable["signatures"])
        tuf.repository_lib._remove_invalid_and_duplicate_signatures(
            root_signable, repository_name
        )
