...
class TestSig(unittest.TestCase):
    ...
    def test_get_signature_status_no_role(self):
        ...
        args = (signable, None)
        self.assertRaises(
            securesystemslib.exceptions.Error, tuf.sig.verify, *args
        )
    ...
    def test_get_signature_status_unknown_signing_scheme(self):
        ...
        self.assertEqual([], sig_status["untrusted_sigs"])
        self.assertEqual(
            [KEYS[0]["keyid"]], sig_status["unknown_signing_schemes"]
        )
    ...
    def test_get_signature_status_below_threshold_unrecognized_sigs(self):
        ...
        signable["signatures"].append(
            securesystemslib.keys.create_signature(KEYS[2], signed)
        )
    ...
    def test_get_signature_status_below_threshold_unauthorized_sigs(self):
        ...
        signable["signatures"].append(
            securesystemslib.keys.create_signature(KEYS[1], signed)
        )
    ...
    def test_generate_rsa_signature(self):
        ...
        returned_signature = tuf.sig.generate_rsa_signature(
            signable["signed"], KEYS[0]
        )
        self.assertTrue(
            securesystemslib.formats.SIGNATURE_SCHEMA.matches(
                returned_signature
            )
        )
