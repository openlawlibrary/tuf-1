...
parser.add_option(
    ...
    "--keys",
    action="store_true",
    dest="should_generate_keys",
    help="Generate a new set of keys",
    default=False,
)
parser.add_option(
    "-d",
    ...
    action="store_true",
    dest="dry_run",
    help="Do not write the files, just run",
    default=False,
)
...
if options.should_generate_keys and not options.dry_run:
    ...
    generate_and_write_ed25519_keypair(
        password="password", filepath=targets_key_file
    )
    generate_and_write_ed25519_keypair(
        password="password", filepath=snapshot_key_file
    )
    generate_and_write_ed25519_keypair(
        password="password", filepath=timestamp_key_file
    )
    generate_and_write_ed25519_keypair(
        password="password", filepath=delegation_key_file
    )
...
timestamp_public = import_ed25519_publickey_from_file(
    timestamp_key_file + ".pub"
)
delegation_public = import_ed25519_publickey_from_file(
    delegation_key_file + ".pub"
)
...
timestamp_private = import_ed25519_privatekey_from_file(
    timestamp_key_file, "password"
)
delegation_private = import_ed25519_privatekey_from_file(
    delegation_key_file, "password"
)
...
repository.targets.add_target(
    os.path.basename(target1_filepath), file_permissions
)
repository.targets.add_target(os.path.basename(target2_filepath))

repository.targets.delegate(
    "role1", [delegation_public], [os.path.basename(target3_filepath)]
)
...
if not options.dry_run:
    create_tuf_client_directory(
        "repository", os.path.join("client", "test_repository1")
    )
