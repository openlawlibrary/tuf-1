...
class TestTutorial(unittest.TestCase):
    ...
    def test_tutorial(self):
        ...
        generate_and_write_ed25519_keypair(
            password="password", filepath="ed25519_key"
        )
        ...
        generate_and_write_rsa_keypair(
            password="password", filepath="targets_key"
        )
        generate_and_write_rsa_keypair(
            password="password", filepath="snapshot_key"
        )
        generate_and_write_rsa_keypair(
            password="password", filepath="timestamp_key"
        )
        ...
        with open(
            os.path.join("repository", "targets", "file1.txt"), "w"
        ) as fobj:
            fobj.write("file1")
        with open(
            os.path.join("repository", "targets", "file2.txt"), "w"
        ) as fobj:
            fobj.write("file2")
        with open(
            os.path.join("repository", "targets", "file3.txt"), "w"
        ) as fobj:
            fobj.write("file3")

        os.mkdir(os.path.join("repository", "targets", "myproject"))
        with open(
            os.path.join("repository", "targets", "myproject", "file4.txt"), "w"
        ) as fobj:
            fobj.write("file4")
        ...
        target4_abspath = os.path.abspath(
            os.path.join("repository", "targets", target4_filepath)
        )
