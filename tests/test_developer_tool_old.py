...
class TestProject(unittest.TestCase):
    ...
    def test_create_new_project(self):
        ...
        project = developer_tool.create_new_project(
            project_name, metadata_directory, location_in_repository
        )
        ...
        self.assertRaises(
            ...
            developer_tool.create_new_project,
            0,
            metadata_directory,
    ...
    def test_add_verification_keys(self):
        ...
        project = developer_tool.Project(
            "test_verification_keys", "somepath", "someotherpath", "prefix"
        )

        # Add invalid verification key.
        self.assertRaises(
            securesystemslib.exceptions.FormatError,
            project.add_verification_key,
            "invalid",
        )
        ...
        #  - load it first
        keystore_path = os.path.join("repository_data", "keystore")
        first_verification_key_path = os.path.join(
            keystore_path, "root_key.pub"
        )
        first_verification_key = developer_tool.import_rsa_publickey_from_file(
            first_verification_key_path
        )

        project.add_verification_key(first_verification_key)

        # Add another verification key (should expect exception.)
        second_verification_key_path = os.path.join(
            keystore_path, "snapshot_key.pub"
        )
        second_verification_key = (
            developer_tool.import_ed25519_publickey_from_file(
                second_verification_key_path
            )
        )
        ...
        project("somesubdelegation").add_verification_key(
    ...
    def test_write(self):
        ...
        delegation_key = developer_tool.import_ed25519_publickey_from_file(
            delegation_key_path
        )

        # Add a subdelegation.
        subdelegation_key_path = os.path.join(
            keystore_path, "timestamp_key.pub"
        )
        subdelegation_key = developer_tool.import_ed25519_publickey_from_file(
            subdelegation_key_path
        )
        ...
        project_private_key = developer_tool.import_rsa_privatekey_from_file(
            project_private_key_path, "password"
        )

        delegation_private_key_path = os.path.join(
            keystore_path, "snapshot_key"
        )
        delegation_private_key = (
            developer_tool.import_ed25519_privatekey_from_file(
                delegation_private_key_path, "password"
            )
        )

        subdelegation_private_key_path = os.path.join(
            keystore_path, "timestamp_key"
        )
        subdelegation_private_key = (
            developer_tool.import_ed25519_privatekey_from_file(
                subdelegation_private_key_path, "password"
            )
        )
        ...
        # + backup delegations.
        delegations_backup = tuf.roledb.get_delegated_rolenames(
        ...
        self.assertEqual(
            list(project.target_files.keys()), list(targets_backup.keys())
        )
